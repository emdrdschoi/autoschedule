...
- Ideal 입력 셀의 `Ideal_Shortfall`을 추가배정 위치 objective에 실제 반영했습니다. 이제 어차피 필요한 추가근무는, 날짜/Duty 분산 penalty와 균형을 이루면서 입력된 Ideal 쪽을 실제로 선호합니다.
- 진단모드의 구간 capacity 계산에도 `maximum_N`, `fixed_N`, `maximum_total`, `fixed_Total` 상한을 반영했습니다.
...
사이드바의 `최대 탐색 시간 (초)` 범위를 기존 최대 300초(5분)에서 **1200초(20분)**까지 확장했습니다.

- 기본값: 60초 — 기존과 동일
- 일반 테스트: 60~180초
- 약 37명 실제 월 스케줄: 300~600초 권장
- `UNKNOWN`이 반복되는 복잡한 조건: 900~1200초까지 시도 가능

`INFEASIBLE`은 탐색시간을 늘려도 해결되지 않는 hard-rule 충돌을 의미합니다.  
`UNKNOWN`은 제한 시간 안에 feasible 해를 찾지 못한 상태이므로 탐색시간을 늘려볼 가치가 있습니다.


---

## V12.7: 사전검사 cache 재사용 + 상단 분석상태 + 실행시간 분리 표시

### 스케줄 생성 버튼에서 0차 사전검사를 다시 강제 실행하지 않음

이전 버전에서는 `스케줄 생성` 버튼을 누를 때:

```python
get_preflight_hard_rule_check(force=True)
```

가 호출되어, 화면에서 이미 같은 hard 설정을 검사한 상태여도 개인별 exact CP-SAT 사전검사를 다시 실행할 수 있었습니다.

V12.7부터는:

```python
get_preflight_hard_rule_check(force=False)
```

를 사용합니다.

즉 **저장된 hard input signature가 같으면 기존 검사 결과를 즉시 재사용**하고,
`fixed_Total`, `minimum_N`, `maximum_N`, rules, requests, PreviousSchedule, Duty Minimum 등
hard input이 실제로 바뀐 경우에만 exact 사전검사를 다시 계산합니다.

`minimum_N`도 hard-preflight signature에 포함했습니다.

### `최대 탐색시간`의 의미

사이드바의 `최대 탐색 시간`은 본 CP-SAT 스케줄 탐색 단계에 적용됩니다.
V12.7에서는 결과 탭 상단에 다음 시간을 따로 표시합니다.

```text
사전검사 | CP-SAT | 후처리 | 전체
```

따라서 예를 들어 `최대 탐색시간=10초`인데 전체가 더 오래 걸린 경우 어느 단계에서 시간이 사용되었는지 확인할 수 있습니다.

### 분석 중 메시지: 화면 최상단 고정

스케줄 생성 중 상태 메시지를 결과 탭 아래쪽이 아니라 **브라우저 화면 최상단에 fixed banner로 표시**합니다.
페이지를 아래로 스크롤한 상태에서 스케줄 생성을 눌러도 계속 보입니다.

단계에 따라 다음처럼 바뀝니다.

```text
⏳ 0차 Hard rule 사전검사 확인 중
⚙️ CP-SAT 근무표 탐색 중
✅ 결과표 / AdditionalAvailability 후처리 중
```

완료 또는 오류 시 banner는 자동으로 사라집니다.
