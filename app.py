...
input[aria-label^="Ideal "] {
    ...
    border: 1px solid rgba(240, 160, 64, 0.90) !important;
    color: #4a2a00 !important;
    -webkit-text-fill-color: #4a2a00 !important;
    caret-color: #4a2a00 !important;
    font-weight: 800 !important;
}
input[aria-label^="Ideal "]::placeholder {
    color: rgba(74, 42, 0, 0.55) !important;
    -webkit-text-fill-color: rgba(74, 42, 0, 0.55) !important;
}
div[data-testid="stNumberInput"]:has(input[aria-label^="Ideal "]) button {
    background: rgba(240, 160, 64, 0.28) !important;
    border-color: rgba(240, 160, 64, 0.72) !important;
    color: #4a2a00 !important;
}
div[data-testid="stNumberInput"]:has(input[aria-label^="Ideal "]) button svg {
    fill: #4a2a00 !important;
    color: #4a2a00 !important;
}
.friendly-note {
...
def _init_state():
    ...
    defaults = {
        ...
        "last_solve_params": None,
        "sol_idx": 0,
        "solved": False,
        "shift_req_version": 0,  # key versioning for shift_request widgets
        "duty_req_version": 0,   # key versioning for minimum duty number_input widgets
        "ideal_duty_req_version": 0, # key versioning for ideal duty number_input widgets
...
DEFAULT_RULES = {
    ...
    "rule_max_consec_days":     5,
    "rule_max_shifts_per_week": 5,
...
def normalize_doctors():
    ...
    for doc in st.session_state.doctors:
        ...
        # V12.13 migration flag. New nurse UI does not auto-create shift_adj.
        # True from an old session means that value was auto-generated and will
        # be reset to 0 by normalize_hidden_shift_adj_values().
        if "shift_adj_auto" not in doc:
            doc["shift_adj_auto"] = None
...
def normalize_maximum_total():
    """Ensure per-doctor maximum_total values exist. -1 means no upper bound."""
    if "maximum_total" not in st.session_state or not isinstance(st.session_state.maximum_total, dict):
        st.session_state.maximum_total = {}
    for ni in range(len(st.session_state.get("doctors", []))):
        st.session_state.maximum_total[ni] = parse_fixed_count_value(
            st.session_state.maximum_total.get(ni, -1)
        )
...
def remove_doctor_and_reindex(remove_idx: int):
    """Delete one doctor and reindex every doctor-indexed session-state structure.

    Doctor identity is index-based throughout the scheduler. A plain doctors.pop(i)
    would shift the doctors list but leave rules/requests/maxima attached to old
    indices, silently assigning one person's settings to another.
    """
    old_doctors = list(st.session_state.get("doctors", []))
    old_count = len(old_doctors)
    if not (0 <= int(remove_idx) < old_count):
        return

    keep_old_indices = [i for i in range(old_count) if i != int(remove_idx)]
    old_to_new = {old_i: new_i for new_i, old_i in enumerate(keep_old_indices)}
    ...
    for key in ("rules", "shift_adj", "shift_counts", "maximum_total", "minimum_N", "maximum_N"):
        ...
        st.session_state[key] = {
            old_to_new[old_i]: old_map[old_i]
            for old_i in keep_old_indices
            if old_i in old_map
        }

    def _reindex_doctor_tuple_map(value):
        if not isinstance(value, dict):
            return {}
        out = {}
        for key, item in value.items():
            # Normal in-app representation: (doctor_idx, day/history_idx)
            if isinstance(key, tuple) and len(key) >= 2:
                ...
                if old_i not in old_to_new:
                    continue
                out[(old_to_new[old_i], *key[1:])] = item
                continue

            # Defensive compatibility for legacy "doctor,day" string keys.
            if isinstance(key, str) and "," in key:
                ...
                if old_i not in old_to_new:
                    continue
                out[f"{old_to_new[old_i]},{rest}"] = item
                continue

            # Unknown key shape: preserve it rather than silently deleting it.
            out[key] = item
        return out

    for key in (
        "base_shift_requests", "fixed_shift_requests", "shift_requests",
        "shift_requests1", "previous_schedule"
    ):
        st.session_state[key] = _reindex_doctor_tuple_map(st.session_state.get(key, {}))

    # Any previous solutions/summaries use the old person indices and are now stale.
    st.session_state.solutions = []
    st.session_state.summaries = []
    st.session_state.additional_availability = []
    st.session_state.last_solve_params = None
    ...
    # Recreate index-based widgets/tables after the deletion.
    for version_key in (
        "shift_req_version", "previous_schedule_version", "shift_count_version", "grade_version"
    ):
...
FIXED_TOTAL_EDITOR_SCHEMA_VERSION = 7  # v7 hides internal shift_adj

def get_fixed_total_editor_key() -> str:
    """Versioned key for the fixed/max count table editor.

    Include a schema version so Streamlit cannot reuse stale data_editor state
    created before columns (such as maximum_total) were added.
    """
    return (
        f"fixed_counts_editor_schema{FIXED_TOTAL_EDITOR_SCHEMA_VERSION}"
        f"_v{st.session_state.get('shift_count_version', 0)}"
    )
...
def normalize_hidden_shift_adj_values():
    """Keep shift_adj as a hidden legacy/advanced soft-balance offset.

    Nurse scheduling no longer auto-generates shift_adj from fixed_Total or annual leave.
    Rows that were auto-generated by V12.13 are reset to 0 during migration.
    Explicit/manual legacy values (shift_adj_auto=False) are preserved.
    """
    if "shift_adj" not in st.session_state or not isinstance(st.session_state.shift_adj, dict):
        st.session_state.shift_adj = {}

    for ni, doc in enumerate(st.session_state.get("doctors", [])):
        current = bounded_int(
            st.session_state.shift_adj.get(ni, doc.get("shift_adj", 0)),
            0, -60, 60,
        )
        if doc.get("shift_adj_auto", None) is True:
            current = 0
        st.session_state.shift_adj[ni] = current
        doc["shift_adj"] = current
        doc["shift_adj_auto"] = False



def pending_fixed_total_editor_df_is_valid(pending_df: object, base_df: pd.DataFrame) -> bool:
    """Check whether a screen-only preview table matches the current table schema.

    Older preview DataFrames did not contain maximum_total.  Checking only No/Name
    allowed that stale preview to replace the new table indefinitely, which made the
    new maximum_total column disappear from the UI.
    """
    ...
    required_cols = (
        ...
        "fixed_D", "fixed_E", "fixed_N",
    )
    if any(col not in pending_df.columns for col in required_cols):
        return False
    if any(col not in base_df.columns for col in required_cols):
        return False

    # Keep column order/schema synchronized too, so newly inserted columns appear
    # exactly where intended rather than being hidden by an old widget state.
    if list(pending_df.columns) != list(base_df.columns):
        return False
...
def get_fixed_total_editor_display_df() -> pd.DataFrame:
    """Return the current monthly-count editor table."""
...
def build_fixed_total_editor_df() -> pd.DataFrame:
    ...
    normalize_minimum_n()
    normalize_maximum_n()
    normalize_hidden_shift_adj_values()
...
def apply_fixed_total_editor_df(edited_df: pd.DataFrame):
    """Apply monthly count settings.

    fixed_Total itself is the workload basis for D/E/N balance.
    No visible/automatic shift_adj recalculation is performed.
    """
    editable_cols = (
        ("fixed_D", "D"), ("fixed_E", "E"),
        ("fixed_N", "N"), ("fixed_Total", "Total"),
    )
...
def render_fixed_total_editor_table():
    ...
    st.markdown(
        ...
        • <b>고정 총근무수</b>: 이번 달 실제 D/E/N 근무수를 정확히 지정하며 D/E/N 균형의 기준이 됩니다.<br>
        • <b>최대 근무수</b>: 이 숫자를 넘지 않도록 제한<br>
        • <b>최소 N근무수 / 최대 N근무수</b>: N근무 범위를 지정<br>
        • 빈칸은 해당 제한을 사용하지 않는다는 뜻입니다.<br>• `a`와 `x`는 쉬는 날짜를 정할 뿐 D/E/N balance 목표를 자동으로 줄이지 않습니다.
        </div>
        """,
        unsafe_allow_html=True,
    )
...
- **D/E/N 균형은 자동 계산**됩니다. `고정 총근무수=7`이면 7근무에 맞는 D/E/N 목표, `21`이면 21근무에 맞는 목표를 자동으로 사용합니다.
- **연차(a)**: 해당 날짜를 쉬게 하는 연차 표시입니다. 총근무수나 D/E/N balance 목표를 자동으로 줄이지 않습니다. 결과의 `Total+연차`에서 기준 working day와 비교할 때 사용합니다.
- **x**: 원하는 OFF 날짜입니다. 근무 날짜 선택에만 영향을 주고 총근무수/D/E/N balance 목표에는 영향을 주지 않습니다.
...
💡 표의 항목 이름에 마우스를 올리면 더 자세한 설명을 볼 수 있습니다.
    ...
    with st.form(f"fixed_counts_editor_form_schema{FIXED_TOTAL_EDITOR_SCHEMA_VERSION}", clear_on_submit=False):
        ...
        edited_df = st.data_editor(
            ...
            column_order=["No", "Name", "maximum_total", "minimum_N", "maximum_N", "fixed_Total", "Grade", "Senior", "Junior", "초저년차", "연차(a)", "fixed_D", "fixed_E", "fixed_N"],
            column_config={
                ...
                "연차(a)": st.column_config.NumberColumn(
                    ...
                    width="small",
                    disabled=True,
                    help="현재 근무 요청에서 a로 입력된 연차 개수입니다. a는 해당 날짜를 OFF로 만들지만 총근무수/D/E/N balance 목표를 자동으로 줄이지 않습니다. 결과의 Total+연차 검산에 사용합니다.",
                ),
...
def render_fixed_total_duty_summary(num_days: int):
    """Render Duty MINIMUM vs exact fixed_total summary."""
    ...
    difference = fixed_sum - minimum_total

    if fixed_total_info["fixed_count"] == 0:
        comment = (
            f"fixed_Total 지정자가 없습니다. Duty 최소인원을 우선 충족하고, "
            f"미지정 {free_count}명은 남은 필요 근무수를 기준으로 자동 목표를 계산해 평준화합니다."
        )
    elif difference > 0:
        comment = (
            f"fixed_Total을 맞추려면 Duty 최소합보다 최소 {difference}근무가 더 필요합니다. "
            "solver가 개인 rule을 만족하는 활성 duty에 자동으로 추가 배정합니다."
        )
    elif difference == 0:
        comment = "fixed_Total 합과 Duty 최소합이 같습니다. 개인 rule을 만족하는 범위에서 최소 인원을 기준으로 배정합니다."
    else:
        shortage = -difference
        if free_count > 0:
            comment = (
                f"fixed_Total 지정자 배정 후 최소 {shortage}근무가 남습니다. "
                f"미지정 {free_count}명이 남은 필요 근무수를 기준으로 나누어 채웁니다."
            )
        else:
            comment = (
                f"모든 인원의 fixed_Total 합이 Duty 최소합보다 {shortage}근무 부족합니다. "
                "현재 값으로는 최소 필요 인원을 채울 수 없습니다."
            )

    doctor_count = len(st.session_state.get("doctors", []))
    duty_average = (minimum_total / doctor_count) if doctor_count > 0 else 0.0
    fixed_count = int(fixed_total_info.get("fixed_count", 0))
    ...
    st.markdown(
        ...
        <div style='font-family:var(--mono);font-size:0.80rem;line-height:1.55;
            ...
            <div>
                ...
                &nbsp;|&nbsp; <span style='color:#ffffff;'>연차(a)</span> <b>{annual_total}</b>
            </div>
    ...
    if fixed_total_info["fixed_count"] > 0 and free_count == 0 and fixed_sum < minimum_total:
        st.error(
            f"모든 인원의 fixed_total 합({fixed_sum})이 Duty 최소합({minimum_total})보다 "
            f"{minimum_total - fixed_sum}개 작습니다. fixed_total 합을 늘리거나 Duty 최소 인원을 줄여야 합니다."
        )
    elif fixed_sum > minimum_total:
        st.info(
            f"fixed_total 때문에 최소 {fixed_sum - minimum_total}개의 추가 배정이 필요합니다. "
            "solver는 Ideal 선호와 날짜/duty 분산을 함께 고려해 추가근무 위치를 정합니다."
        )
...
def duty_value_from_row(row, shift_label: str, fallback_pos: int) -> int:
    """Read MINIMUM D/E/N from current or older DutyRequests sheets."""
    if row is None:
        return 1
    aliases = [
        shift_label, shift_label.upper(), shift_label.lower(),
        f"Min_{shift_label}", f"MIN_{shift_label}", f"Minimal_{shift_label}",
        f"minimum_{shift_label}", f"최소_{shift_label}",
    ]
...
def _run_exact_hard_conflict_diagnostics(combined_req: dict) -> tuple[pd.DataFrame, str]:
    """Call scheduler.py's conservative exact hard-input conflict checker."""
    try:
        import importlib.util
        scheduler_path = Path(__file__).resolve().with_name("scheduler.py")
        if not scheduler_path.exists():
            return pd.DataFrame(), f"scheduler.py를 찾을 수 없습니다: {scheduler_path}"
        spec = importlib.util.spec_from_file_location("autoscheduler_diag_scheduler_v9", scheduler_path)
        if spec is None or spec.loader is None:
            return pd.DataFrame(), f"scheduler.py를 불러올 수 없습니다: {scheduler_path}"
        scheduler_module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(scheduler_module)
        diagnose_hard_conflicts = getattr(scheduler_module, "diagnose_hard_conflicts", None)
        ...
        params = {
            ...
            "ideal_duty_requests": st.session_state.get("ideal_duty_requests", {}),
...
def run_hard_bottleneck_diagnostics() -> dict:
    ...
    combined_req = refresh_combined_shift_requests()
    exact_conflicts_df, exact_conflicts_error = _run_exact_hard_conflict_diagnostics(combined_req)
    ...
    for di in range(n_days):
        ...
        for si, shift_key in enumerate(shift_keys):
            ...
            issues = []
            status = "가능"
            # DutyRequests are MINIMUM staffing. More hard-fixed staff than the
            # minimum is allowed; it simply creates legitimate overstaffing.
    ...
    for length in window_lengths:
        for start_di in range(0, n_days - length + 1):
            ...
            for ni in range(n_docs):
                ...
                fixed_total = parse_fixed_count_value(st.session_state.shift_counts.get(ni, {}).get("Total", -1))
                max_total = parse_fixed_count_value(st.session_state.maximum_total.get(ni, -1))
                if fixed_total >= 0:
                    person_possible = min(person_possible, fixed_total)
                elif max_total >= 0:
                    person_possible = min(person_possible, max_total)
                ...
                for si, sk in enumerate(shift_keys):
                    ...
                    if sk == "N":
                        avail_shift_days = min(
                            avail_shift_days,
                            _diagnostic_max_shift_count_without_long_blocks(length, n_max)
                        )
                        ...
                        if fixed_n >= 0:
                            avail_shift_days = min(avail_shift_days, fixed_n)
                        elif max_n >= 0:
                            avail_shift_days = min(avail_shift_days, max_n)
                    if length <= 7 and r6 > 0:
    ...
    return {
        "exact_conflicts_df": exact_conflicts_df,
        "exact_conflicts_error": exact_conflicts_error,
        "duty_df": duty_df,
        "fixed_df": fixed_df,
        "window_df": window_df,
        "generated_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "note": (
            "0차는 PreviousSchedule·대문자 D/E/N·x/a·fixed/maximum_total/maximum_N·개인 sequence rule 사이의 확정 충돌과, "
            "fixed_Total이 실제 개인 hard rule 아래에서 정확히 달성 가능한지를 CP-SAT로 검사합니다. "
            "1차/2차는 후보 수와 구간 capacity를 보는 병목 검사이므로 원인 후보로 해석하세요."
        ),
    }
...
def render_hard_bottleneck_diagnostics(results: dict):
    ...
    exact_df = results.get("exact_conflicts_df", pd.DataFrame())
    exact_error = results.get("exact_conflicts_error", "")
    ...
    st.markdown("**0차 진단 · hard coding 충돌 + fixed_Total 정확 달성 가능성**")
    st.caption(
        "대문자 D/E/N은 solver에서 반드시 배정되는 hard-fixed 근무로 해석됩니다. "
        "직전 5일 스케줄과 N-rest/N-gap, E→D, 연속근무, sliding 7일 상한, fixed/maximum_total/maximum_N을 서로 대조하고, "
        "fixed_Total이 있는 사람은 실제 개인 hard rule과 동일한 CP-SAT 모델로 그 정확한 총근무수가 가능한지도 검사합니다."
    )
    if exact_error:
        st.warning(exact_error)
    elif exact_df is None or exact_df.empty:
        st.success("직접적인 hard coding 충돌이나 개인 fixed_Total 달성 불가 문제는 발견되지 않았습니다.")
    else:
        st.error(f"확정 충돌 {len(exact_df)}건이 발견되었습니다. 아래 항목은 현재 입력 그대로는 동시에 만족할 수 없습니다.")
    ...
    st.markdown("**1차 진단 · 개인 fixed_D/E/N 단순 가능성**")
    if fixed_df is None or fixed_df.empty:
        st.success("개인 fixed_D/E/N이 단순 가능 최대를 초과하는 경우는 발견되지 않았습니다.")
    else:
...
def run_hard_rule_diagnostics() -> dict:
    ...
    for di in range(num_days):
        ...
        for si, shift_label in enumerate(SHIFT_SHORT):
            ...
            # DutyRequests are minimum staffing; hard-fixed count may exceed `need`.
...
def _parse_result_shift_codes(value) -> list[str]:
    text = str(value or "").strip().upper()
    return [s for s in ("D", "E", "N") if s in text]
...
def build_schedule_excel_bytes(
    *,
    ...
    additional_availability: dict | None = None,
) -> bytes:
    ...
    availability_rows = []
    additional_availability = additional_availability or {}
    for ni in display_order:
        ...
        for di in range(num_days):
            ...
            availability_rows.append({
                ...
                "Day": get_day_label(start_date, di),
                "Current": (sol.get((ni, di), "") or "").upper(),
    ...
    # Staffing transparency: export actual vs Minimal vs Ideal for every date/shift.
    duty_staffing_rows = []
    shift_keys = ["D", "E", "N"]
    for di in range(num_days):
        d = start_date + timedelta(days=di)
        ...
        for si, shift_key in enumerate(shift_keys):
            ...
            actual = sum(
                1 for ni in range(len(doctors))
                if shift_key in _parse_result_shift_codes(sol.get((ni, di), ""))
            )
            duty_staffing_rows.append({
                "Date": d.strftime("%Y-%m-%d"),
                "Day": get_day_label(start_date, di),
                "Duty": shift_key,
                "Actual": actual,
    ...
    with pd.ExcelWriter(towrite, engine="openpyxl") as writer:
        ...
        duty_staffing_df.to_excel(writer, sheet_name="DutyStaffing", index=False)
            ...
            try:
                ...
                if df_doctors is not None and not df_doctors.empty:
                    for row_pos, row in df_doctors.iterrows():
                        ...
                        auto_raw = row_get_any(row, ['shift_adj_auto', '근무조정 자동', 'auto_shift_adj'], None)
                        if auto_raw is None or (isinstance(auto_raw, float) and pd.isna(auto_raw)):
                            adj_auto = False
                        elif isinstance(auto_raw, str):
                            adj_auto = auto_raw.strip().lower() in {"1", "true", "yes", "y", "auto", "자동"}
                        else:
                            adj_auto = bool(auto_raw)
                        new_doctors.append({
                            "name": name,
                            "shift_adj": adj,
                            "shift_adj_auto": adj_auto,
                            "grade": grade,
                        })
                else:
                    fallback_names = derive_doctor_names_from_old_config(df_rules, df_shift, df_fixed_shift, df_previous)
                    new_doctors = [
                        {"name": name, "shift_adj": 0, "shift_adj_auto": False, "grade": DEFAULT_DOCTOR_GRADE}
                ...
                if df_duty is not None and not df_duty.empty:
                    ...
                    for i in range(len(df_duty)):
                        ...
                        loaded_duty[i] = mins
                        loaded_ideal_duty[i] = [
                            ideal_duty_value_from_row(row, 'D', mins[0]),
                            ideal_duty_value_from_row(row, 'E', mins[1]),
                            ideal_duty_value_from_row(row, 'N', mins[2]),
                        ]
                else:
...
with tab1:
    ...
    with st.form("shift_requests_form", clear_on_submit=False):
        ...
        st.markdown("**날짜 유형 & 개인별 근무 요청** · `d/e/n` = 해당 근무 불가 | `D/E/N` = 반드시 원하는 근무 | `x` = 원하는 OFF | `a` = 연차(OFF + 결과 검산용)")
...
with tab2:
    with st.form("duty_settings_form", clear_on_submit=False):
        ...
        for chunk_start in range(0, num_days, CHUNK):
            ...
            # Minimal D/E/N rows (hard lower bounds)
...
with tab3:
    ...
    st.markdown(
        ...
        ① <b>모든 간호사에게 고정 총근무수가 입력된 경우</b>: 전체 근무수는 고정 총근무수의 합으로 정해집니다. 각 사람의 D/E/N 균형도 자신의 고정 총근무수에 비례해 자동 계산됩니다(예: 7근무인 사람은 7근무 기준, 21근무인 사람은 21근무 기준). 날짜별 최소 인원을 먼저 채우고, 남는 근무는 Ideal을 우선 고려하면서 분산합니다.<br>
        ② <b>고정 총근무수가 비어 있는 간호사가 있는 경우</b>: 고정값이 있는 사람은 그 숫자를 정확히 지키고, 남은 필요 근무수를 고정값이 없는 사람들에게 나누어 자동 총근무 목표를 만든 뒤 그 목표를 기준으로 D/E/N 균형을 계산합니다.
        </div>
        """, unsafe_allow_html=True,
    )
    ...
    if save_grade_rule_settings:
        st.session_state["grade_rule_version"] = st.session_state.get("grade_rule_version", 0) + 1
        st.session_state["grade_version"] = st.session_state.get("grade_version", 0) + 1
        # maximum_total is also editable in this form. Recreate the top fixed/max
    ...
    if not doctors:
        ...
        for ni, doc in enumerate(doctors):
            ...
            if fixed_val >= 0 and max_val >= 0 and fixed_val > max_val:
                ...
                break
            fixed_shift_sum = sum(
                parse_fixed_count_value(st.session_state.shift_counts.get(ni, {}).get(sk, -1))
                for sk in ("D", "E", "N")
                if parse_fixed_count_value(st.session_state.shift_counts.get(ni, {}).get(sk, -1)) >= 0
            )
            if max_val >= 0 and fixed_shift_sum > max_val:
                precheck_error = (
                    f"{doc['name']}의 fixed_D/E/N 지정 합({fixed_shift_sum})이 maximum_total({max_val})보다 큽니다."
                )
                break
        ...
        if not precheck_error:
            ...
            for ni in range(len(doctors)):
                ...
                max_val = parse_fixed_count_value(st.session_state.maximum_total.get(ni, -1))
                if fixed_val >= 0:
                    upper_bounds.append(fixed_val)
                elif max_val >= 0:
                    upper_bounds.append(max_val)
                else:
                    all_bounded = False
                    break
            if all_bounded and sum(upper_bounds) < fixed_total_info["total_duty"]:
                precheck_error = (
        ...
        # fixed_total sum ABOVE the Duty minimum is valid and will be absorbed by
        # overstaffing active duties.  Only an all-fixed total BELOW the minimum is
        # immediately impossible.
        if (
            not precheck_error
            and fixed_total_info["fixed_count"] > 0
            and fixed_total_info["free_count"] == 0
            and fixed_total_info["fixed_sum"] < fixed_total_info["total_duty"]
        ):
            precheck_error = (
                f"모든 인원의 fixed_total 합({fixed_total_info['fixed_sum']})이 "
                f"Duty 최소합({fixed_total_info['total_duty']})보다 "
                f"{fixed_total_info['total_duty'] - fixed_total_info['fixed_sum']}개 작습니다. "
                "최소 필요 인원을 채울 수 없습니다."
            )
        ...
        if precheck_error:
            ...
            with st.spinner("최적 스케줄을 계산 중입니다..."):
                try:
                    ...
                    # This avoids accidentally importing an older scheduler.py from
                    # another folder or a stale module left in Python's import cache.
                    import importlib.util
                    scheduler_path = Path(__file__).resolve().with_name("scheduler.py")
                    if not scheduler_path.exists():
                        raise ImportError(f"같은 폴더에 scheduler.py가 없습니다: {scheduler_path}")

                    spec = importlib.util.spec_from_file_location("autoscheduler_local_scheduler_v9", scheduler_path)
                    if spec is None or spec.loader is None:
                    ...
                    if actual_scheduler_api != expected_scheduler_api:
                        raise ImportError(
                            "app.py와 scheduler.py 버전이 맞지 않습니다. "
                            f"현재 app 기대 버전={expected_scheduler_api}, "
                            f"scheduler 버전={actual_scheduler_api or '구버전/표시없음'}. "
                            f"반드시 같은 압축파일에 들어 있는 두 파일을 함께 사용하세요. "
                            f"읽은 scheduler 경로: {scheduler_path}"
                        )

                    build_and_solve = getattr(scheduler_module, "build_and_solve", None)
                    evaluate_additional_availability = getattr(scheduler_module, "evaluate_additional_availability", None)
                    diagnose_hard_conflicts = getattr(scheduler_module, "diagnose_hard_conflicts", None)
                    if not callable(build_and_solve) or not callable(evaluate_additional_availability) or not callable(diagnose_hard_conflicts):
                        raise ImportError(
                            "scheduler.py에 필요한 함수가 없습니다: "
                            "build_and_solve / evaluate_additional_availability / diagnose_hard_conflicts. "
                            f"읽은 파일: {scheduler_path}"
                        )

                    # shift_adj is hidden/internal only. fixed_Total itself drives
                    # workload-based D/E/N balancing.
                    normalize_hidden_shift_adj_values()
                    ...
                    params = {
                        ...
                        "ideal_duty_requests": st.session_state.ideal_duty_requests,
...
with tab5:
    if not st.session_state.solved or not st.session_state.solutions:
        ...
        st.caption("Total은 실제 D/E/N 근무수입니다. `Total+연차`는 실제 근무수에 a를 더한 검산값으로, 개인의 기준 working day와 비교할 때 사용할 수 있습니다. x는 이 값에 더하지 않습니다.")
        # 최대 근무수 / 최소 N / 최대 N은 설정·검증용 값이므로
        # 최종 근무 통계표에서는 숨겨 결과를 간단하게 보여준다.
        summary_view = summary_display.drop(
            columns=["maximum_total", "minimum_N", "maximum_N"],
            errors="ignore",
        ...
        availability = {}
        availability_list = st.session_state.get("additional_availability", [])
        ...
        for ni in display_order:
            ...
            for di in range(num_days):
                candidates = availability.get((ni, di), [])
                if not candidates:
                    continue
                available_date_count += 1
                d = start_date + timedelta(days=di)
                day_lbl = get_day_label(start_date, di)
                cand_text = "/".join(candidates)
                entries.append(f"{d.day}{day_lbl} {cand_text}")
                availability_detail_rows.append({
                    "No": ni + 1,
                    "Name": doctors[ni]["name"],
                    "날짜": d.strftime("%Y-%m-%d"),
                    "요일": day_lbl,
                    "현재 근무": (sol.get((ni, di), "") or "·").upper(),
                    "추가 가능": cand_text,
                })
        ...
        st.dataframe(availability_summary_df, use_container_width=True, hide_index=True)
        if availability_detail_rows:
            with st.expander("추가 근무 가능일 상세 보기"):
                st.dataframe(pd.DataFrame(availability_detail_rows), use_container_width=True, hide_index=True)
        ...
        if nav_col5.button("Excel 준비", use_container_width=True, key=f"prepare_excel_{sol_idx}"):
            excel_bytes = build_schedule_excel_bytes(
                ...
                additional_availability=availability,
            )
        ...
        st.caption("각 날짜·D/E/N별 실제 / Minimal / Ideal 인원을 비교합니다. Minimal은 hard, Ideal은 추가배정 위치를 정하는 soft 선호값입니다.")
        ...
        for di in range(num_days):
            ...
            for si, (shift_key, shift_name) in enumerate(shift_labels):
                staff = [ni for ni in range(len(doctors)) if shift_key.lower() in str(sol.get((ni, di), '')).lower()]
